...
// const { UpcomingCompany } = require("../models/UpcomingCompany");
import { UpcomingCompany } from "../models/UpcomingCompany.js";
    ...
    for (const company of companies) {
      ...
      const companyInfo = {
        ...
        applicants: applicants.map((applicant) => ({
          ...
          stream: applicant.stream, // Add this line to include department
        })),
      };
...
router.get("/upcomingCompanies", async (req, res) => {
  try {
...
// GET a single upcoming company by ID
router.get("/upcomingCompanies/:id", async (req, res) => {
  try {
    const company = await UpcomingCompany.findById(req.params.id);
    if (!company) {
      return res.status(404).json({ message: "Company not found" });
    }
    res.json(company);
...
// DELETE an upcoming company by ID
router.delete("/upcomingCompanies/:id", async (req, res) => {
  try {
    const company = await UpcomingCompany.findByIdAndDelete(req.params.id);
    if (!company) {
      return res.status(404).json({ message: "Company not found" });
    }
...
// POST a new upcoming company (for Add functionality)
router.post("/addUpcomingCompany", async (req, res) => {
  try {
    const { companyName, eligibilityCriteria, about } = req.body;
    const newCompany = new UpcomingCompany({
      companyName,
      eligibilityCriteria,
      about,
    });
    ...
    console.error("Error adding upcoming company:", error);
    res.status(500).json({ message: "Internal Server Error" });
  }
});

// PUT (Update) an upcoming company by ID
router.put("/upcomingCompanies/:id", async (req, res) => {
  try {
    const { companyName, eligibilityCriteria, about } = req.body;
    const company = await UpcomingCompany.findByIdAndUpdate(
      req.params.id,
      { companyName, eligibilityCriteria, about },
